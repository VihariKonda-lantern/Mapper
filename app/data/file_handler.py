...
def has_header(file: IO[bytes], delimiter: str = ",", sample_size: int = 2048) -> bool:
    ...
    Uses multiple heuristics for more reliable detection:
    1. csv.Sniffer().has_header (primary)
    2. Checks if first row looks like column names (contains header keywords, mostly strings)
    3. Compares first row with second row to see if second looks like data
    ...
    try:
        # Method 1: Use csv.Sniffer (original method)
        sniffer = csv.Sniffer()
        content = file.read(sample_size)
        file.seek(position)
        encoding = detect_encoding(content, sample_size)
        sample = content.decode(encoding, errors="ignore")
        sniffer_result = sniffer.has_header(sample)
        
        # Method 2: Analyze first few rows directly for more reliability
        file.seek(position)
        # Create a new BytesIO from the content we already read
        content_io = io.BytesIO(content)
        text_io = io.TextIOWrapper(content_io, encoding=encoding, errors='ignore')
        reader = csv.reader(text_io, delimiter=delimiter)
        
        try:
            first_row = next(reader)
            second_row = next(reader) if sample.count('\n') > 1 else None
        except StopIteration:
            file.seek(position)
            return sniffer_result  # Fall back to sniffer result
        ...
        # Check 1: First row contains common header keywords
        header_keywords = ['id', 'name', 'date', 'code', 'amount', 'field', 'column', 'type', 'value', 'description']
        first_row_lower = [str(cell).strip().lower() for cell in first_row if cell]
        keyword_matches = sum(1 for kw in header_keywords if any(kw in cell for cell in first_row_lower))
        if keyword_matches > 0:
            header_indicators += 1
        total_checks += 1
        
        # Check 2: First row values are mostly non-numeric strings
        # csv.reader returns strings, so we only need to check if they're numeric
        non_numeric_count = sum(
            1 for cell in first_row 
            if cell and not cell.strip().replace('.', '').replace('-', '').replace('+', '').isdigit()
        )
        if non_numeric_count > len(first_row) * 0.7:  # At least 70% non-numeric
            header_indicators += 1
        total_checks += 1
        
        # Check 3: First row has unique values (headers are usually unique)
        if len(first_row) == len(set(cell.strip().lower() for cell in first_row if cell)):
            header_indicators += 1
        total_checks += 1
        
        # Check 4: If second row exists, compare it with first row
        if second_row:
            # If second row has more numeric values than first, first is likely header
            first_numeric = sum(
                1 for cell in first_row 
                if cell and cell.strip().replace('.', '').replace('-', '').replace('+', '').isdigit()
            )
            second_numeric = sum(
        ...
        # If majority of checks indicate header, return True
        # Also trust sniffer if it says True (more conservative)
        heuristic_result = (header_indicators / total_checks) >= 0.5 if total_checks > 0 else False
        
        # Combine results: if either sniffer or heuristics say header, return True
        # This makes detection more lenient (less false negatives)
        return sniffer_result or heuristic_result
        
    except Exception:
        file.seek(position)
        # If all detection fails, be more lenient and assume headers exist
        # This prevents false negatives (saying no headers when headers exist)
        return True
...
def load_source_file(file: Any) -> Tuple[Any, bool]:
    """Load an uploaded source file and return parsed data plus header flag.
    
    Generic file loader that supports all formats (CSV, TXT, TSV, XLSX, XLS, JSON, PARQUET).
...
def load_claims_file(file: Any) -> Tuple[Any, bool]:
    ...
    Note: This is an alias for load_source_file() for backward compatibility.
