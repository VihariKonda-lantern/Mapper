...
def create_heatmap(df: pd.DataFrame, title: str = "", use_container_width: bool = True) -> None:
    ...
    if PLOTLY_AVAILABLE:
        ...
        if numeric_df.empty:
            ...
            try:
                numeric_df = df.astype(float, errors='ignore')
            except Exception:
                st.warning("Cannot create heatmap: DataFrame contains no numeric data")
                return
